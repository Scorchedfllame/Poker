...
import packages.requests.adapters  # pylint: disable=ungrouped-imports
import packages.requests.exceptions  # pylint: disable=ungrouped-imports
...
from packages.google.oauth2 import service_account
...
class TimeoutGuard(object):
    ...
    def __init__(self, timeout, timeout_error_type=packages.requests.exceptions.Timeout):
...
class Request(transport.Request):
    ...
    def __call__(
        ...
        **kwargs
        ...
        except packages.requests.exceptions.RequestException as caught_exc:
            new_exc = exceptions.TransportError(caught_exc)
            raise new_exc from caught_exc


class _MutualTlsAdapter(packages.requests.adapters.HTTPAdapter):
    """
    ...
    def __init__(self, cert, key):
        ...
        import packages.urllib3.contrib.pyopenssl  # type: ignore
...
class _MutualTlsOffloadAdapter(packages.requests.adapters.HTTPAdapter):
    """
    ...
    def __init__(self, enterprise_cert_file_path):
        import certifi
        import packages.urllib3.contrib.pyopenssl

        from packages.google.auth.transport import _custom_tls_signer
...
class AuthorizedSession(requests.Session):
    ...
    def __init__(
        ...
        if auth_request is None:
            ...
            retry_adapter = packages.requests.adapters.HTTPAdapter(max_retries=3)
