...
def _warn_about_problematic_credentials(credentials):
    ...
    """
    from packages.google.auth import _cloud_sdk
...
def _get_gae_credentials():
    ...
    try:
        ...
        import packages.google.auth.app_engine as app_engine
    except ImportError:
...
def _get_external_account_credentials(
    ...
    if info.get("subject_token_type") == _AWS_SUBJECT_TOKEN_TYPE:
        ...
        from packages.google.auth import pluggable
...
def _get_external_account_authorized_user_credentials(
    ...
    try:
        from packages.google.auth import external_account_authorized_user
...
def _get_authorized_user_credentials(filename, info, scopes=None):
    from packages.google.oauth2 import credentials
...
def _get_impersonated_service_account_credentials(filename, info, scopes):
    from packages.google.auth import impersonated_credentials
...
def _get_gdch_service_account_credentials(filename, info):
    from packages.google.oauth2 import gdch_credentials
...
def _apply_quota_project_id(credentials, quota_project_id):
    ...
    from packages.google.oauth2 import credentials as authorized_user_credentials
