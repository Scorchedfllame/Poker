...
#
# Last 2 Digits + infinite decimals: Kicker, High Card sometimes


def set_in_hand(cards: list[Card], in_hand=True) -> None:
    for card in cards:
        card.in_hand = in_hand


def check_flush(cards: list[Card]) -> int | bool:
...
def check_straight_flush(cards: list[Card]) -> int | bool:
...
def check_straight(cards: list[Card]) -> int | bool:
    ...
    if 2 in nums:
        for i in range(len(nums)):
            if nums[i] == 1:
                cards[i].value = 14
        return 5000000 + 500
    return get_highest_card(cards).value * 100 + 5000000


def check_n_of_a_kind(n: int, cards: list[Card], amount=1) -> list[list[Card]] | bool:
    highest = []
...
def check_4_of_a_kind(cards: list[Card]) -> int | bool:
    matches = check_n_of_a_kind(4, cards)
    if matches:
        set_in_hand(matches[0])
        return matches[0][0].value * 100 + 8000000
    return False


def check_3_of_a_kind(cards: list[Card]) -> int | bool:
    matches = check_n_of_a_kind(3, cards)
    if matches:
        set_in_hand(matches[0])
        return matches[0][0].value * 100 + 4000000
    return False


def check_pair(cards: list[Card]) -> int | bool:
    matches = check_n_of_a_kind(2, cards)
    if matches:
        set_in_hand(matches[0])
        return matches[0][0].value * 100 + 2000000
    return False


def check_2_pair(cards: list[Card]) -> int | bool:
    firsts = check_n_of_a_kind(2, cards, amount=2)
    if firsts:
        if len(firsts) == 2 and firsts[0] and firsts[1]:
            set_in_hand(firsts[1] + firsts[0])
            return firsts[0][0].value * 10000 + firsts[1][1].value * 100 + 3000000
    return False


def check_full_house(cards: list[Card]) -> int | bool:
    confirm_three = False
    confirm_two = False
    total = 0
...
def check_high_card(cards: list[Card]) -> int:
    total = 0
    multiplier = 5
    while [card for card in cards if card.in_hand is False]:
        high = get_highest_card([card for card in cards if card.in_hand is False])
        high.in_hand = True
...
def add_kicker(cards: list[Card]) -> float | bool:
    if cards:
        return (check_high_card(cards) - 1000000) / 100
    return False


def check_five(cards: list, size: int, checker) -> float:
    val = []
...
def check_for_all(cards: list, size: int = 5) -> float:
    ...
    for i in functions:
        ...
        if curr:
            maximum.append(curr)
    return float(sorted(maximum, reverse=True)[0])
